...
# просто цикл на всех вершинах
G_ordinary: nx.Graph = nx.cycle_graph(graph_size)
...
# клики по 4 вершины, в каждой будет по 1 заражённому
G_home = nx.Graph()
G_home.add_nodes_from(list(range(graph_size)))
for i in range(0, graph_size, 4):
    next_click_size = 4
    if i + 4 > graph_size - 1:
...
init_distr = [NodeStates.Infected if i % 4 == 0 else NodeStates.Sucept for i in range(len(G_ordinary.nodes))]

epidemic = EpidemicWithLockdown(G_ordinary, init_distr, G_home, 0.2, 0.7, 0.7)
sampler = SamplerWithLockdown(epidemic, (1, 10))
...
for time in range(25):
    # выводим графы в первый день локдауна
